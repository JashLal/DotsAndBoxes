            ...
            for row in range(self.backend.row_bound + 1):
                for col in range(self.backend.column_bound + 1):
                    ...
                    if not row_is_even == col_is_even and not self.backend.edge_taken(row, col):
                        ...
                        move_advantage, moves = self._min_max(depth - 1, same_turn)
                        if move_advantage > max_advantage:
                            max_advantage = move_advantage
                            optimal_moves = moves
                            optimal_moves.insert(0, ("C", row, col))
                        # return to original state
                        self.backend.revert_move(row, col)
            return max_advantage, optimal_moves
        # user move
        else:
            ...
            for row in range(self.backend.row_bound + 1):
                for col in range(self.backend.column_bound + 1):
                    ...
                    if not row_is_even == col_is_even and not self.backend.edge_taken(row, col):
                        ...
                        move_advantage, moves = self._min_max(depth - 1, not same_turn)
                        if move_advantage < min_advantage:
                            min_advantage = move_advantage
                            optimal_moves = moves
                            optimal_moves.insert(0, ("U", row, col))
                        # return to original state
                        self.backend.revert_move(row, col)
            return min_advantage, optimal_moves
...
#         advantage, moves = computer._min_max(8, True)
#         print(advantage)
#         print(moves)
